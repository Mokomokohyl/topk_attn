...
// ============================================================================
// Retrieval Attention Kernel with Global Memory (No DSM)
// ============================================================================
template<int TOPK_PER_BLOCK>
__cluster_dims__(1, CLUSTER_SIZE, 1)
__global__ void retrieval_attention_global_kernel(
    ...
    if (is_qk_block) {
        ...
        // Write sorted results to shared memory (only need top K)
        __syncthreads();
        for (int item = 0; item < ITEMS_PER_THREAD; ++item) {
            int rank = tid * ITEMS_PER_THREAD + item;
            if (rank < TOPK) {
    ...
    // Sync cluster to ensure QK blocks have written to global memory
    cluster.sync();
    
    // PV block: read from global memory, then compute softmax and P@V
    if (is_pv_block) {
        // Data layout: [all_scores][all_indices][probs][output][reduction]
        half* s_all_scores = (half*)shared_mem;
        int* s_all_indices = (int*)&s_all_scores[TOTAL_TOPK];
        half* s_probs = (half*)&s_all_indices[TOTAL_TOPK];
        half* s_output = &s_probs[TOTAL_TOPK];
...
// ============================================================================
// Retrieval Attention Kernel with Pipelined Batch Processing (Global Buffer)
// ============================================================================
template<int TOPK_PER_BLOCK>
__cluster_dims__(1, CLUSTER_SIZE, 1)
__global__ void retrieval_attention_pipelined_global_kernel(
    const half* __restrict__ query,      // [BATCH_SIZE, NUM_HEADS, HEAD_DIM]
    const half* __restrict__ key_cache,  // [SEQ_LEN, NUM_HEADS, HEAD_DIM]
    const half* __restrict__ value_cache,// [SEQ_LEN, NUM_HEADS, HEAD_DIM]
    half* __restrict__ output,           // [BATCH_SIZE, NUM_HEADS, HEAD_DIM]
    int batch_size,
    half* __restrict__ d_global_scores,
    int* __restrict__ d_global_indices
) {
    using Config = KernelConfig<TOPK_PER_BLOCK>;
    constexpr int TOPK = Config::TOPK;
    constexpr int TOTAL_TOPK = Config::TOTAL_TOPK;
    ...
    if (is_qk_block) {
        ...
        for (int b = 0; b < batch_size; ++b) {
            ...
            int offset = head_idx * TOTAL_TOPK + cluster_rank * TOPK;
            for (int i = tid; i < TOPK; i += BLOCK_SIZE) {
                d_global_scores[offset + i] = __float2half(s_scores[i]);
            }
            __threadfence_block();
            
            for (int i = tid; i < TOPK; i += BLOCK_SIZE) {
                int local_idx = s_indices[i];
                d_global_indices[offset + i] = key_start + local_idx;
            }
            __threadfence_block();
    ...
    if (is_pv_block) {
        ...
        for (int b = 0; b < batch_size; ++b) {
            ...
            int offset = head_idx * TOTAL_TOPK;
            for (int i = tid; i < TOTAL_TOPK; i += BLOCK_SIZE) {
                s_all_scores[i] = d_global_scores[offset + i];
            }
            __threadfence_block();
            
            for (int i = tid; i < TOTAL_TOPK; i += BLOCK_SIZE) {
                s_all_indices[i] = d_global_indices[offset + i];
            }
            __threadfence_block();
...
// ============================================================================
// Standalone TopK Kernel (Step 1 of 2)
// ============================================================================
template<int TOPK_PER_BLOCK>
__global__ void retrieval_topk_kernel(
    const half* __restrict__ query,      // [BATCH_SIZE, NUM_HEADS, HEAD_DIM]
    const half* __restrict__ key_cache,  // [SEQ_LEN, NUM_HEADS, HEAD_DIM]
    half* __restrict__ global_scores,    // [BATCH_SIZE, NUM_HEADS, TOTAL_TOPK]
    int* __restrict__ global_indices     // [BATCH_SIZE, NUM_HEADS, TOTAL_TOPK]
    ...
    // Grid: (BATCH * NUM_HEADS, QK_BLOCKS_PER_CLUSTER)
    int batch_head_idx = blockIdx.x;
    int chunk_idx = blockIdx.y; // 0 to QK_BLOCKS_PER_CLUSTER-1
    ...
    // Use float for scores to support CUB sort
    extern __shared__ float s_scores[]; // Size: KEYS_PER_BLOCK * sizeof(float)
    int* s_indices = (int*)&s_scores[KEYS_PER_BLOCK];
    ...
    // Compute scores for this block's key range
    int key_start = chunk_idx * KEYS_PER_BLOCK;
    ...
    // Write to Global Memory
    int global_offset = batch_head_idx * TOTAL_TOPK + chunk_idx * TOPK;
    for (int i = tid; i < TOPK; i += BLOCK_SIZE) {
        global_scores[global_offset + i] = __float2half(s_scores[i]);
        global_indices[global_offset + i] = key_start + s_indices[i];
    }
...
// ============================================================================
// Standalone Reduce Kernel (Step 2 of 2)
// ============================================================================
template<int TOPK_PER_BLOCK>
__global__ void retrieval_reduce_kernel(
    const half* __restrict__ value_cache,// [SEQ_LEN, NUM_HEADS, HEAD_DIM]
    half* __restrict__ output,           // [BATCH_SIZE, NUM_HEADS, HEAD_DIM]
    const half* __restrict__ global_scores,    // [BATCH_SIZE, NUM_HEADS, TOTAL_TOPK]
    const int* __restrict__ global_indices     // [BATCH_SIZE, NUM_HEADS, TOTAL_TOPK]
) {
    using Config = KernelConfig<TOPK_PER_BLOCK>;
    constexpr int TOTAL_TOPK = Config::TOTAL_TOPK;
    constexpr int BLOCK_SIZE = Config::BLOCK_SIZE;
    ...
    // Shared memory
    extern __shared__ half shared_mem[];
    half* s_all_scores = shared_mem;
    int* s_all_indices = (int*)&s_all_scores[TOTAL_TOPK];
    half* s_probs = (half*)&s_all_indices[TOTAL_TOPK];
    half* s_output = &s_probs[TOTAL_TOPK];
    
    // Read from Global Memory
    int global_offset = batch_head_idx * TOTAL_TOPK;
    for (int i = tid; i < TOTAL_TOPK; i += block_size) {
        s_all_scores[i] = global_scores[global_offset + i];
        s_all_indices[i] = global_indices[global_offset + i];
    }
...
__global__ void gather_kv_kernel(
    const half* __restrict__ key_cache,   // [SEQ_LEN, NUM_HEADS, HEAD_DIM]
    const half* __restrict__ value_cache, // [SEQ_LEN, NUM_HEADS, HEAD_DIM]
    const int* __restrict__ indices,      // [BATCH_SIZE, NUM_HEADS, TOTAL_TOPK]
    half* __restrict__ gathered_key,      // [BATCH_SIZE, TOTAL_TOPK, NUM_HEADS, HEAD_DIM]
    half* __restrict__ gathered_value,    // [BATCH_SIZE, TOTAL_TOPK, NUM_HEADS, HEAD_DIM]
    int total_topk
) {
    // Grid: (BATCH * NUM_HEADS, TOTAL_TOPK)
    int batch_head_idx = blockIdx.x;
    int topk_idx = blockIdx.y;
    
    int batch_idx = batch_head_idx / NUM_HEADS;
    int head_idx = batch_head_idx % NUM_HEADS;
    int tid = threadIdx.x; // Loop over HEAD_DIM
    
    if (topk_idx >= total_topk) return;
    
    // Read index
    int global_idx_offset = batch_head_idx * total_topk + topk_idx;
    int key_idx = indices[global_idx_offset];
    
    // Pointers
    // Original: [SEQ_LEN, NUM_HEADS, HEAD_DIM]
    // Stride: NUM_HEADS * HEAD_DIM
    const half* k_src = key_cache + key_idx * NUM_HEADS * HEAD_DIM + head_idx * HEAD_DIM;
    const half* v_src = value_cache + key_idx * NUM_HEADS * HEAD_DIM + head_idx * HEAD_DIM;
    
    // Gathered: [BATCH, TOTAL_TOPK, NUM_HEADS, HEAD_DIM]
    // Stride for batch: TOTAL_TOPK * NUM_HEADS * HEAD_DIM
    // Stride for topk: NUM_HEADS * HEAD_DIM
    // Stride for head: HEAD_DIM
    long long batch_offset = (long long)batch_idx * total_topk * NUM_HEADS * HEAD_DIM;
    long long topk_offset = (long long)topk_idx * NUM_HEADS * HEAD_DIM;
    long long head_offset = (long long)head_idx * HEAD_DIM;
    
    half* k_dst = gathered_key + batch_offset + topk_offset + head_offset;
    half* v_dst = gathered_value + batch_offset + topk_offset + head_offset;
...
template<int TOPK_PER_BLOCK>
void run_retrieval_attention_gather_baseline(
    const half* d_query,
    ...
    // Allocate temporary global memory
    half* d_global_scores;
    int* d_global_indices;
    half* d_gathered_key;
    half* d_gathered_value;
    ...
    size_t gathered_size = batch_size * TOTAL_TOPK * NUM_HEADS * HEAD_DIM * sizeof(half);
    
    CUDA_CHECK(cudaMalloc(&d_global_scores, scores_size));
    CUDA_CHECK(cudaMalloc(&d_global_indices, indices_size));
    CUDA_CHECK(cudaMalloc(&d_gathered_key, gathered_size));
    CUDA_CHECK(cudaMalloc(&d_gathered_value, gathered_size));
    
    // Shared memory sizes
    // TopK Kernel: scores + indices + sort storage
    using BlockSort = cub::BlockRadixSort<float, BLOCK_SIZE, Config::ITEMS_PER_THREAD, int>;
    size_t topk_shared_size = KEYS_PER_BLOCK * sizeof(float) + 
                              KEYS_PER_BLOCK * sizeof(int) + 
                              sizeof(typename BlockSort::TempStorage);
                              
    // Baseline Kernel Shared Memory
    size_t baseline_shared_size = (TOTAL_TOPK + HEAD_DIM) * sizeof(half) + 256 * sizeof(half);
    
    printf("Retrieval Attention Gather+Baseline Config: TOPK=%d, TOTAL_TOPK=%d\n", TOPK, TOTAL_TOPK);
    
    // Grids
    dim3 grid_topk(batch_size * NUM_HEADS, QK_BLOCKS_PER_CLUSTER);
    dim3 block_topk(BLOCK_SIZE);
    
    dim3 grid_gather(batch_size * NUM_HEADS, TOTAL_TOPK);
    dim3 block_gather(128); // Enough for HEAD_DIM=128
    
    dim3 grid_baseline(NUM_HEADS); // Per batch
    dim3 block_baseline(256);
    
    // Warmup
    for (int i = 0; i < 10; i++) {
        // 1. TopK
        retrieval_topk_kernel<TOPK_PER_BLOCK><<<grid_topk, block_topk, topk_shared_size>>>(
            d_query, d_key_cache, d_global_scores, d_global_indices
        );
        
        // 2. Gather
        gather_kv_kernel<<<grid_gather, block_gather>>>(
            d_key_cache, d_value_cache, d_global_indices, d_gathered_key, d_gathered_value, TOTAL_TOPK
        );
        
        // 3. Baseline (Loop over batch)
        for (int b = 0; b < batch_size; b++) {
            const half* q_ptr = d_query + b * NUM_HEADS * HEAD_DIM;
            const half* k_ptr = d_gathered_key + b * TOTAL_TOPK * NUM_HEADS * HEAD_DIM;
            const half* v_ptr = d_gathered_value + b * TOTAL_TOPK * NUM_HEADS * HEAD_DIM;
            half* out_ptr = d_output + b * NUM_HEADS * HEAD_DIM;
            
            baseline_attention_kernel<<<grid_baseline, block_baseline, baseline_shared_size>>>(
                q_ptr, k_ptr, v_ptr, out_ptr, TOTAL_TOPK
            );
        }
    }
    ...
    printf("Retrieval Attention (Gather+Baseline) - Avg time: %.3f ms, Throughput: %.2f iter/s\n",
    ...
    CUDA_CHECK(cudaFree(d_global_scores));
    CUDA_CHECK(cudaFree(d_global_indices));
    CUDA_CHECK(cudaFree(d_gathered_key));
    CUDA_CHECK(cudaFree(d_gathered_value));
}
...
template<int TOPK_PER_BLOCK>
void run_retrieval_attention_global(
    const half* d_query,
    const half* d_key_cache,
    const half* d_value_cache,
    half* d_output,
    ...
    // Calculate shared memory size
    // QK blocks: scores(float) + topk_indices(int) + CUB storage
    using BlockSort = cub::BlockRadixSort<float, BLOCK_SIZE, Config::ITEMS_PER_THREAD, int>;
    
    size_t qk_shared_size = TOPK * sizeof(float) + // s_scores (output)
                            TOPK * sizeof(int) +   // s_indices (output)
                            sizeof(typename BlockSort::TempStorage); // CUB storage

    // PV blocks: all_scores + all_indices + probs + output + reduction
    size_t pv_shared_size = TOTAL_TOPK * sizeof(half) +  // all_scores
                           TOTAL_TOPK * sizeof(int) +   // all_indices
                           TOTAL_TOPK * sizeof(half) +  // probs
                           HEAD_DIM * sizeof(half) +    // output
                           32 * sizeof(half);           // reduction workspace
    ...
    printf("Retrieval Attention (Global Memory) Config: TOPK=%d, TOTAL_TOPK=%d, BLOCK_SIZE=%d\n", 
           TOPK, TOTAL_TOPK, BLOCK_SIZE);
    printf("Shared memory per block: %zu bytes (%.1f KB)\n", shared_mem_size, shared_mem_size / 1024.0f);
    ...
    // Set shared memory config
    CUDA_CHECK(cudaFuncSetAttribute(
        retrieval_attention_global_kernel<TOPK_PER_BLOCK>,
        cudaFuncAttributeMaxDynamicSharedMemorySize,
        shared_mem_size
    ));
    ...
    for (int i = 0; i < 10; i++) {
        retrieval_attention_global_kernel<TOPK_PER_BLOCK><<<grid, block, shared_mem_size>>>(
            d_query, d_key_cache, d_value_cache, d_output, d_global_scores, d_global_indices
        );
    }
    CUDA_CHECK(cudaDeviceSynchronize());
    ...
    printf("Retrieval Attention (Global) - Avg time: %.3f ms, Throughput: %.2f iter/s\n",
           milliseconds / num_iterations,
           num_iterations * 1000.0f / milliseconds);
...
template<int TOPK_PER_BLOCK>
void run_retrieval_attention_pipelined_global(
    const half* d_query,
    const half* d_key_cache,
    const half* d_value_cache,
    half* d_output,
    ...
    // Allocate temporary global memory for scores and indices
    half* d_global_scores;
    int* d_global_indices;
    ...
    CUDA_CHECK(cudaFuncSetAttribute(
        retrieval_attention_pipelined_global_kernel<TOPK_PER_BLOCK>,
        cudaFuncAttributeMaxDynamicSharedMemorySize,
        shared_mem_size
    ));
    ...
    // Warmup
    for (int i = 0; i < 10; i++) {
        retrieval_attention_pipelined_global_kernel<TOPK_PER_BLOCK><<<grid, block, shared_mem_size>>>(
            d_query, d_key_cache, d_value_cache, d_output, batch_size, d_global_scores, d_global_indices
        );
    }
    CUDA_CHECK(cudaDeviceSynchronize());
...
template<int TOPK_PER_BLOCK>
void run_retrieval_attention_two_kernels(
    const half* d_query,
    const half* d_key_cache,
    const half* d_value_cache,
    half* d_output,
    ...
    // Reduce Kernel: scores + indices + probs + output
    size_t reduce_shared_size = TOTAL_TOPK * sizeof(half) + 
                                TOTAL_TOPK * sizeof(int) + 
                                TOTAL_TOPK * sizeof(half) + 
                                HEAD_DIM * sizeof(half);
    
    printf("Retrieval Attention Two Kernels Config: TOPK=%d, TOTAL_TOPK=%d\n", TOPK, TOTAL_TOPK);
    ...
    for (int i = 0; i < 10; i++) {
        ...
        retrieval_reduce_kernel<TOPK_PER_BLOCK><<<grid_reduce, block_reduce, reduce_shared_size>>>(
            d_value_cache, d_output, d_global_scores, d_global_indices
        );
    }
    CUDA_CHECK(cudaDeviceSynchronize());
    ...
    printf("Retrieval Attention (Two Kernels) - Avg time: %.3f ms, Throughput: %.2f iter/s\n",
           milliseconds / num_iterations,
           num_iterations * 1000.0f / milliseconds);
...
void launch_retrieval_attention_gather_baseline_32(
    ...
    run_retrieval_attention_gather_baseline<32>(query, key_cache, value_cache, output, batch_size, 1);
}
...
void launch_retrieval_attention_gather_baseline_128(
    ...
    run_retrieval_attention_gather_baseline<128>(query, key_cache, value_cache, output, batch_size, 1);
}
...
void launch_retrieval_attention_gather_baseline_256(
    ...
    run_retrieval_attention_gather_baseline<256>(query, key_cache, value_cache, output, batch_size, 1);
}
...
void launch_retrieval_attention_gather_baseline_512(
    ...
    run_retrieval_attention_gather_baseline<512>(query, key_cache, value_cache, output, batch_size, 1);
}
...
void launch_retrieval_attention_gather_baseline_1024(
    ...
    run_retrieval_attention_gather_baseline<1024>(query, key_cache, value_cache, output, batch_size, 1);
}
...
void launch_retrieval_attention_pipelined_global_32(
    const half* query,
    const half* key_cache,
    const half* value_cache,
    half* output,
    ...
    run_retrieval_attention_pipelined_global<32>(query, key_cache, value_cache, output, batch_size, 1);
}

void launch_retrieval_attention_pipelined_global_128(
    const half* query,
    const half* key_cache,
    const half* value_cache,
    half* output,
    ...
    run_retrieval_attention_pipelined_global<128>(query, key_cache, value_cache, output, batch_size, 1);
}

void launch_retrieval_attention_pipelined_global_256(
    const half* query,
    const half* key_cache,
    const half* value_cache,
    half* output,
    ...
    run_retrieval_attention_pipelined_global<256>(query, key_cache, value_cache, output, batch_size, 1);
}

void launch_retrieval_attention_pipelined_global_512(
    const half* query,
    const half* key_cache,
    const half* value_cache,
    half* output,
    ...
    run_retrieval_attention_pipelined_global<512>(query, key_cache, value_cache, output, batch_size, 1);
}

void launch_retrieval_attention_pipelined_global_1024(
    const half* query,
    const half* key_cache,
    const half* value_cache,
    half* output,
    ...
    run_retrieval_attention_pipelined_global<1024>(query, key_cache, value_cache, output, batch_size, 1);
}
...
void launch_retrieval_attention_two_kernels_32(
    ...
    run_retrieval_attention_two_kernels<32>(query, key_cache, value_cache, output, batch_size, 1);
}
...
void launch_retrieval_attention_two_kernels_128(
    ...
    run_retrieval_attention_two_kernels<128>(query, key_cache, value_cache, output, batch_size, 1);
}
...
void launch_retrieval_attention_two_kernels_256(
    ...
    run_retrieval_attention_two_kernels<256>(query, key_cache, value_cache, output, batch_size, 1);
}
...
void launch_retrieval_attention_two_kernels_512(
    ...
    run_retrieval_attention_two_kernels<512>(query, key_cache, value_cache, output, batch_size, 1);
}
...
void launch_retrieval_attention_two_kernels_1024(
    ...
    run_retrieval_attention_two_kernels<1024>(query, key_cache, value_cache, output, batch_size, 1);
}
...
void run_baseline_attention(
    ...
    for (int i = 0; i < 10; i++) {
        baseline_attention_kernel<<<NUM_HEADS, BLOCK_SIZE, shared_mem_size>>>(
            d_query, d_key_cache, d_value_cache, d_output, SEQ_LEN
        );
    }
    CUDA_CHECK(cudaDeviceSynchronize());
...
int main() {
    ...
    printf("\n=== Running Retrieval Attention (Global Memory, TOPK=128 per block) ===\n");
    run_retrieval_attention_global<128>(d_query, d_key_cache, d_value_cache, d_output);
